...
function test_verify_manifest_missing_signature() {
  ...
  ok(!packagedAppUtils.verifyManifest(header_missing_signature, manifest),
...
function test_verify_manifest_invalid_signature() {
  packagedAppUtils = Cc["@mozilla.org/network/packaged-app-utils;1"]
               .createInstance(Ci.nsIPackagedAppUtils);
  ok(!packagedAppUtils.verifyManifest(header_invalid_signature, manifest),
...
function test_verify_manifest_malformed_json() {
  packagedAppUtils = Cc["@mozilla.org/network/packaged-app-utils;1"]
               .createInstance(Ci.nsIPackagedAppUtils);
  ok(!packagedAppUtils.verifyManifest(header, manifest_malformed_json),
...
function test_verify_manifest_missing_moz_resources() {
  packagedAppUtils = Cc["@mozilla.org/network/packaged-app-utils;1"]
               .createInstance(Ci.nsIPackagedAppUtils);
  ok(!packagedAppUtils.verifyManifest(header, manifest_missing_moz_resources),
...
function test_verify_manifest_success() {
  packagedAppUtils = Cc["@mozilla.org/network/packaged-app-utils;1"]
               .createInstance(Ci.nsIPackagedAppUtils);
  ok(packagedAppUtils.verifyManifest(header, manifest),
...
function test_check_integrity_success() {
  ...
  for (let resource of JSON.parse(manifestBody)["moz-resources"]) {
    ok(packagedAppUtils.checkIntegrity(resource.src, resource.integrity),
...
function test_check_integrity_filename_not_matched() {
  ok(!packagedAppUtils.checkIntegrity("/nosuchfile.html", "sha256-kass...eoirW-e"),
     "mismatched filename should fail integrity check");
  run_next_test();
}

function test_check_integrity_hashvalue_not_matched() {
  ok(!packagedAppUtils.checkIntegrity("/index.html", "kass...eoirW-e"),
